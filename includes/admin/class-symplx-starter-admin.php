...
class Symplx_Motion_Admin {
    ...
    public function register_settings() {
        register_setting( 'symplx_motion', 'symplx_motion_provider', [
        ...
        register_setting( 'symplx_motion', 'symplx_motion_default_mode', [
            'sanitize_callback' => function( $v ) { return in_array( $v, [ 'off', 'select', 'all' ], true ) ? $v : 'off'; },
            'default'           => 'off',
        ] );

        register_setting( 'symplx_motion', 'symplx_motion_default_effect', [
            'sanitize_callback' => function( $v ) { return in_array( $v, [ 'kenburns', 'parallax' ], true ) ? $v : 'kenburns'; },
            'default'           => 'kenburns',
        ] );

        register_setting( 'symplx_motion', 'symplx_motion_max_video_size_mb', [
        ...
        register_setting( 'symplx_motion', 'symplx_motion_replicate_api_token', [
            'sanitize_callback' => function( $value ) {
                $v = is_string( $value ) ? trim( $value ) : '';
                if ( $v === '' ) return get_option( 'symplx_motion_replicate_api_token', '' ); // keep existing
        ...
        register_setting( 'symplx_motion', 'symplx_motion_replicate_model_version', [
        ...
        add_settings_section(
            'symplx_motion_main',
            __( 'Main Settings', 'symplx-motion' ),
            '__return_false',
            'symplx_motion'
        );

        add_settings_field(
            'symplx_motion_provider',
            __( 'Provider', 'symplx-motion' ),
            [ $this, 'field_provider' ],
            'symplx_motion',
            'symplx_motion_main'
        );
        ...
        add_settings_field(
            'symplx_motion_replicate_api_token',
            __( 'Replicate API Token', 'symplx-motion' ),
        ...
        add_settings_field(
            'symplx_motion_replicate_model_version',
            __( 'Replicate Model Version', 'symplx-motion' ),
        ...
        register_setting( 'symplx_motion', 'symplx_motion_replicate_image_key', [ 'sanitize_callback' => 'sanitize_text_field', 'default' => 'input_image' ] );
        register_setting( 'symplx_motion', 'symplx_motion_replicate_fps_key', [ 'sanitize_callback' => 'sanitize_text_field', 'default' => 'fps' ] );
        register_setting( 'symplx_motion', 'symplx_motion_replicate_frames_key', [ 'sanitize_callback' => 'sanitize_text_field', 'default' => 'num_frames' ] );
        register_setting( 'symplx_motion', 'symplx_motion_replicate_duration_key', [ 'sanitize_callback' => 'sanitize_text_field', 'default' => '' ] );
        register_setting( 'symplx_motion', 'symplx_motion_replicate_prompt_key', [ 'sanitize_callback' => 'sanitize_text_field', 'default' => 'prompt' ] );
        register_setting( 'symplx_motion', 'symplx_motion_replicate_extra_input', [
            'sanitize_callback' => function( $v ) {
        ...
        add_settings_field(
            ...
            __( 'Replicate Input Mapping', 'symplx-motion' ),
        ...
        add_settings_field(
            'symplx_motion_default_mode',
            __( 'Default Mode', 'symplx-motion' ),
        ...
        add_settings_field(
            'symplx_motion_default_effect',
            __( 'Default Effect', 'symplx-motion' ),
        ...
        add_settings_field(
            ...
            __( 'Max Video Size (MB)', 'symplx-motion' ),
    ...
    public function field_provider() {
        $value = get_option( 'symplx_motion_provider', 'mock' );
        echo '<select name="symplx_motion_provider">';
        ...
        echo '</select>';
        echo '<p class="description">' . esc_html__( 'Mock provider uses CSS to simulate motion. Replicate uses hosted models for image→video. If Action Scheduler is installed, background polling runs via it; otherwise WP‑Cron is used.', 'symplx-motion' ) . '</p>';
    }

    public function field_replicate_token() {
        $stored = get_option( 'symplx_motion_replicate_api_token', '' );
        // Do not echo the token back. Leave blank to keep, enter new to replace, type "reset" to clear.
        echo '<input type="password" class="regular-text" name="symplx_motion_replicate_api_token" value="" autocomplete="off" placeholder="••••••" />';
        echo '<p class="description">' . esc_html__( 'Enter to set/replace. Leave blank to keep current. Type "reset" to clear.', 'symplx-motion' ) . '</p>';
    }
    ...
    public function field_replicate_input_mapping() {
        $image = get_option( 'symplx_motion_replicate_image_key', 'input_image' );
        $fps = get_option( 'symplx_motion_replicate_fps_key', 'fps' );
        $frames = get_option( 'symplx_motion_replicate_frames_key', 'num_frames' );
        $duration = get_option( 'symplx_motion_replicate_duration_key', '' );
        $prompt = get_option( 'symplx_motion_replicate_prompt_key', 'prompt' );
        $extra = get_option( 'symplx_motion_replicate_extra_input', '' );
        echo '<p><label>' . esc_html__( 'Image key', 'symplx-motion' ) . ' <input type="text" name="symplx_motion_replicate_image_key" value="' . esc_attr( $image ) . '" class="regular-text" /></label></p>';
        echo '<p><label>' . esc_html__( 'FPS key', 'symplx-motion' ) . ' <input type="text" name="symplx_motion_replicate_fps_key" value="' . esc_attr( $fps ) . '" class="regular-text" /></label></p>';
        echo '<p><label>' . esc_html__( 'Frames key', 'symplx-motion' ) . ' <input type="text" name="symplx_motion_replicate_frames_key" value="' . esc_attr( $frames ) . '" class="regular-text" /></label></p>';
        echo '<p><label>' . esc_html__( 'Duration key (optional)', 'symplx-motion' ) . ' <input type="text" name="symplx_motion_replicate_duration_key" value="' . esc_attr( $duration ) . '" class="regular-text" /></label></p>';
        echo '<p><label>' . esc_html__( 'Prompt key', 'symplx-motion' ) . ' <input type="text" name="symplx_motion_replicate_prompt_key" value="' . esc_attr( $prompt ) . '" class="regular-text" /></label></p>';
        echo '<p><label>' . esc_html__( 'Extra input JSON', 'symplx-motion' ) . '<br/>';
        echo '<textarea name="symplx_motion_replicate_extra_input" rows="4" class="large-text code" placeholder="{\n  &quot;strength&quot;: 0.7\n}">' . esc_textarea( $extra ) . '</textarea></label></p>';
        echo '<p class="description">' . esc_html__( 'For bytedance/seedance-1-pro, set Image key to "image". If the model expects a different frames/duration key, set it here. Extra input JSON merges into the payload.', 'symplx-motion' ) . '</p>';
    }

    public function field_default_mode() {
        $value = get_option( 'symplx_motion_default_mode', 'off' );
        echo '<select name="symplx_motion_default_mode">';
        ...
        echo '</select>';
        echo '<p class="description">' . esc_html__( 'Default behavior for new posts. Authors can override per post or per image.', 'symplx-motion' ) . '</p>';
    }

    public function field_default_effect() {
        $value = get_option( 'symplx_motion_default_effect', 'kenburns' );
        echo '<select name="symplx_motion_default_effect">';
        foreach ( [ 'kenburns' => 'Ken Burns', 'parallax' => 'Parallax' ] as $k => $label ) {
            echo '<option value="' . esc_attr( $k ) . '"' . selected( $value, $k, false ) . '>' . esc_html( $label ) . '</option>';
        }
        echo '</select>';
        echo '<p class="description">' . esc_html__( 'Visual effect used when motion is enabled and no AI asset is available.', 'symplx-motion' ) . '</p>';
    }

    public function field_max_video_size() {
        $value = (int) get_option( 'symplx_motion_max_video_size_mb', 100 );
        echo '<input type="number" min="1" max="2048" name="symplx_motion_max_video_size_mb" value="' . esc_attr( $value ) . '" />';
        echo '<p class="description">' . esc_html__( 'Reject generated videos larger than this size for security/performance. Default 100MB.', 'symplx-motion' ) . '</p>';
    }
    ...
    public function render_settings() {
        ...
        echo '<div class="wrap">';
        echo '<h1>' . esc_html__( 'Symplx Motion Settings', 'symplx-motion' ) . '</h1>';
        ...
        settings_fields( 'symplx_motion' );
        do_settings_sections( 'symplx_motion' );
        submit_button();
        echo '</form>';

        // Presets for Replicate input mapping
        echo '<hr style="margin:20px 0;" />';
        echo '<h2>' . esc_html__( 'Replicate Presets', 'symplx-motion' ) . '</h2>';
        echo '<p>' . esc_html__( 'Quickly apply recommended input mappings for popular models.', 'symplx-motion' ) . '</p>';
        ...
        echo '<label for="symplx_preset">' . esc_html__( 'Preset', 'symplx-motion' ) . '</label>';
        echo '<select id="symplx_preset" name="preset">';
        echo '<option value="svd">' . esc_html__( 'Stable Video Diffusion (img2vid)', 'symplx-motion' ) . '</option>';
        echo '<option value="seedance">' . esc_html__( 'Seedance (bytedance/seedance-1-pro)', 'symplx-motion' ) . '</option>';
        echo '<option value="minimax">' . esc_html__( 'MiniMax (minimax/video-01)', 'symplx-motion' ) . '</option>';
        echo '</select>';
        submit_button( __( 'Apply Preset', 'symplx-motion' ), 'secondary', 'submit', false );
        echo '</form>';

        // Provider connection test form (non-destructive)
        echo '<hr style="margin:20px 0;" />';
        echo '<h2>' . esc_html__( 'Provider Connection Test', 'symplx-motion' ) . '</h2>';
        echo '<p>' . esc_html__( 'Verify credentials and model configuration without generating a job.', 'symplx-motion' ) . '</p>';
        echo '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '">';
        echo '<input type="hidden" name="action" value="symplx_test_provider" />';
        wp_nonce_field( 'symplx_test_provider' );
        submit_button( __( 'Test Provider Connection', 'symplx-motion' ), 'secondary' );
        echo '</form>';
        echo '</div>';
    }
    ...
    public function handle_apply_replicate_preset() {
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_die( __( 'Permission denied', 'symplx-motion' ) );
        }
        ...
        switch ( $preset ) {
            case 'seedance':
                update_option( 'symplx_motion_replicate_image_key', 'image' );
                update_option( 'symplx_motion_replicate_fps_key', 'fps' );
                update_option( 'symplx_motion_replicate_frames_key', '' );
                update_option( 'symplx_motion_replicate_duration_key', 'duration' );
                update_option( 'symplx_motion_replicate_prompt_key', 'prompt' );
                update_option( 'symplx_motion_replicate_extra_input', '' );
                $messages[] = __( 'Applied Seedance preset: image key set to "image", using fps + duration.', 'symplx-motion' );
                break;
            case 'minimax':
                ...
                update_option( 'symplx_motion_replicate_prompt_key', 'prompt' );
                update_option( 'symplx_motion_replicate_extra_input', '' );
                $messages[] = __( 'Applied MiniMax preset: image key set to "image", using fps + duration. Update model version to minimax/video-01:<version>.', 'symplx-motion' );
                break;
            case 'svd':
            default:
                update_option( 'symplx_motion_replicate_image_key', 'input_image' );
                update_option( 'symplx_motion_replicate_fps_key', 'fps' );
                update_option( 'symplx_motion_replicate_frames_key', 'num_frames' );
                update_option( 'symplx_motion_replicate_duration_key', '' );
                update_option( 'symplx_motion_replicate_prompt_key', 'prompt' );
                update_option( 'symplx_motion_replicate_extra_input', '' );
                $messages[] = __( 'Applied SVD preset: input_image, fps and num_frames mapping set.', 'symplx-motion' );
                break;
        }
        $key = 'symplx_preset_' . wp_generate_password( 8, false, false );
        set_transient( $key, [ 'type' => 'success', 'messages' => $messages ], 60 );
        $redirect = add_query_arg( [ 'page' => 'symplx-motion', 'symplx_test_key' => $key ], admin_url( 'options-general.php' ) );
    ...
    public function handle_generate_post_images() {
        ...
        foreach ( $ids as $att_id ) {
            ...
            if ( ! $resolved_model ) { $resolved_model = get_option( 'symplx_motion_replicate_model_version', '' ); }
    ...
    public function maybe_bulk_notice() {
        ...
        $msg = sprintf( _n( '%d job started.', '%d jobs started.', $ok, 'symplx-motion' ), $ok );
        if ( $fail ) {
            $msg .= ' ' . sprintf( _n( '%d failed.', '%d failed.', $fail, 'symplx-motion' ), $fail );
        }
    ...
    public function handle_test_provider() {
        ...
        if ( 'replicate' === $provider ) {
            ...
            if ( ! $token ) {
                ...
                if ( is_wp_error( $res ) ) {
                    $messages[] = sprintf( __( 'Token check failed: %s', 'symplx-motion' ), $res->get_error_message() );
                    $type = 'error';
                } else {
                    $code = wp_remote_retrieve_response_code( $res );
                    if ( $code === 200 ) {
                        $messages[] = __( 'Replicate token OK.', 'symplx-motion' );
                    } elseif ( $code === 401 ) {
                        $messages[] = __( 'Replicate token invalid (401).', 'symplx-motion' );
                        $type = 'error';
                    } else {
                        $messages[] = sprintf( __( 'Replicate token check returned HTTP %d.', 'symplx-motion' ), $code );
            ...
            if ( $version ) {
                ...
                if ( preg_match( '/^([\w-]+)\/([\w-]+):([\w-]+)/', $version, $m ) ) {
                    ...
                    if ( is_wp_error( $res2 ) ) {
                        $messages[] = sprintf( __( 'Model version check failed: %s', 'symplx-motion' ), $res2->get_error_message() );
                        $type = 'error';
                    } else {
                        $code2 = wp_remote_retrieve_response_code( $res2 );
                        if ( $code2 === 200 ) {
                            $messages[] = __( 'Replicate model version found.', 'symplx-motion' );
                        } elseif ( $code2 === 404 ) {
                            $messages[] = __( 'Model/version not found (404). Check owner/model and version hash.', 'symplx-motion' );
                            $type = 'error';
                        } else {
                            $messages[] = sprintf( __( 'Model version check returned HTTP %d.', 'symplx-motion' ), $code2 );
            ...
            $messages[] = __( 'Using Mock provider: no external connection required.', 'symplx-motion' );
        }
    ...
    public function render_post_metabox( $post ) {
        ...
        echo '<p><label><input type="checkbox" name="symplx_motion_enable_all" value="1"' . checked( $enable, true, false ) . '> ' . esc_html__( 'Enable motion for all images in this post', 'symplx-motion' ) . '</label></p>';
        echo '<p><label>' . esc_html__( 'Default prompt (optional)', 'symplx-motion' ) . '<br/>';
        echo '<textarea name="symplx_motion_default_prompt" rows="3" style="width:100%">' . esc_textarea( $prompt ) . '</textarea>';
        echo '</label></p>';
        echo '<hr />';
        echo '<p><strong>' . esc_html__( 'Per‑post Replicate overrides (optional)', 'symplx-motion' ) . '</strong></p>';
        echo '<p><label>' . esc_html__( 'Model version', 'symplx-motion' ) . '<br/>';
        ...
        echo '</label></p>';
        echo '<p><label>' . esc_html__( 'Mapping preset', 'symplx-motion' ) . '<br/>';
        $presets = [
            '' => __( 'Use global mapping', 'symplx-motion' ),
        ...
        echo '</select>';
        echo '</label></p>';
        echo '<p class="description">' . esc_html__( 'Use the [symplx_motion id=...] shortcode to target specific images if not enabling all.', 'symplx-motion' ) . '</p>';
        ...
        echo '<p><a href="' . esc_url( $bulk_url ) . '" class="button button-primary">' . esc_html__( 'Generate motion for all images in this post', 'symplx-motion' ) . '</a></p>';
    }
